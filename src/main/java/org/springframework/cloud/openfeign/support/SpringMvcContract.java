...
//				}
//			}
            FeignClient classAnnotation = findMergedAnnotation(clz,
                    FeignClient.class);
            if (classAnnotation != null) {
                // Prepend path from class annotation if specified
                if (StringUtils.hasText(classAnnotation.path())) {
                    String pathValue = emptyToNull(classAnnotation.path());
                    pathValue = resolve(pathValue);
                    if (!pathValue.startsWith("/")) {
                        pathValue = "/" + pathValue;
                    }
    ...
    private void parseProduces(MethodMetadata md, Method method,
        ...
        if (clientAccepts != null) {
            ...
            md.template().header(ACCEPT, MediaType.APPLICATION_JSON_VALUE);
        }
    }
    ...
    private void parseConsumes(MethodMetadata md, Method method,
        ...
        if (clientProduces != null) {
            ...
            md.template().header(CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE);
        }
    }
